...
while True:
    ...
    # Save the current frame when 'S' key is pressed
    key = cv2.waitKey(1)
    if key & 0xFF == ord('s'):
        filename = f"matrix_image_{int(time.time())}.png"
        cv2.imwrite(filename, matrix_window)
        print(f"Image saved as {filename}")
    elif key & 0xFF == 27:  # Exit when 'ESC' key is pressed
        break
